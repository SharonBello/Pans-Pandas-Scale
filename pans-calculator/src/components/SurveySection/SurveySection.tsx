...
const SurveySection: React.FC<SurveySectionProps> = ({ title, items, onComplete }) => {
  ...
  // אם יש Modal פתוח, נסמן אותו לפי אינדקס השאלה בתוך ה־answers; אחרת null
  const [editIndex, setEditIndex] = useState<number | null>(null);

  const total = items.length;

  /** מעדכן אחד מהטווחים (before, after או current) של שאלת ה־id */
  ...
  /** מנקה תהיות TypeScript: מוסיף פעולה לצורך בניית מפתח השדה */
  ...
  /** פותח את המודל לעריכה של השאלה עם האינדקס הנתון */
  const handleEditClick = (index: number) => {
    setEditIndex(index);
  };

  /** סוגר את המודל ושומר את הערכים המעודכנים בתוך answers (ללא שינוי מרירת מסך הדו־שיח) */
